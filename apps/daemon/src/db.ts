...
function migrate(db) {
  db.exec(`
    ...
    -- Reference library (T2.1). User-curated taste signals scoped to a
    -- project. \`kind\` discriminates the value type:
    --   'design-system' → value is a design-systems/* slug
    --   'screenshot'    → value is a relative path under .od/refs/<project>/
    --   'url'           → value is the source URL
    --   'figma'         → value is a Figma file/node URL or fileKey:nodeId
    -- Future kinds are added without a migration; the daemon ignores
    ...
    CREATE INDEX IF NOT EXISTS idx_project_refs
      ON project_references(project_id, created_at DESC);

    -- Taste memory (T2.2). Derived signals about what the user accepts
    -- and rejects. Each row is an *event* — aggregation into preferences
    -- happens on read so we never need a batch derivation step.
    --
    --   scope       'user' | 'project' | 'session'
    --   project_id  null when scope='user' (user-level prefs span projects)
    --   session_id  null unless scope='session'
    --   subject     normalised key e.g. 'color:#6366f1', 'skill:deck',
    --               'design-system:stripe', 'mood:editorial'
    --   polarity    +1 (positive signal) or -1 (negative signal)
    --   source      where the signal came from: 'judge', 'lint', 'auto-revise',
    --               'comment', 'reference', 'feedback'
    --   payload     small JSON blob with optional context (rationale, etc.)
    --
    -- Privacy: this table lives in the local SQLite. Topology B does
    -- not upload it. The web "memory inspector" UI lets the user view
    -- and delete any row.
    ...
    CREATE INDEX IF NOT EXISTS idx_taste_user_subject
      ON taste_signals(scope, subject, created_at DESC)
      WHERE scope = 'user';
    CREATE INDEX IF NOT EXISTS idx_taste_project_subject
      ON taste_signals(project_id, subject, created_at DESC)
      WHERE scope = 'project';
    CREATE INDEX IF NOT EXISTS idx_taste_session
      ON taste_signals(session_id, created_at DESC)
      WHERE scope = 'session';
  `);
