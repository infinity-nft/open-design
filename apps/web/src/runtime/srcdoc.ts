...
<html>
  ...
  // CSP must be the first thing in <head> — directives only apply to
  // resources the parser sees AFTER the meta tag. We inject before any
  // other transformation so the storage shim, deck bridge, comment bridge,
  // and any user script are all governed by the policy.
  const withCsp = injectCsp(wrapped);
...
// the document is an opaque origin and cannot read parent storage. The
// remaining attack surfaces for AI-generated artifacts are
//   1. network exfiltration via fetch / XHR / WebSocket / EventSource /
//      Beacon — closed by `connect-src 'none'`,
//   2. form-based exfiltration — closed by `form-action 'none'`,
//   3. clickjacking pivot via nested iframes — closed by `frame-src 'none'`,
//   4. plugin / object / worker abuse — closed by their respective directives.
// We deliberately allow https: for script/style/font/img so legitimate
// previews keep working (Tailwind CDN, Google Fonts, branded imagery,
// React UMD for the JSX surface). Tightening any of those breaks more
// previews than it secures; users who need a strict policy can opt in
// via a future `strict` mode.
const PREVIEW_CSP = [
  "default-src 'none'",
