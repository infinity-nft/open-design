...
// Threat model is the same as `srcdoc.ts` but tighter on connect-src and
// form-action because the JSX surface never legitimately needs network
// access from inside the preview — Babel.transform runs locally on text
// the host already has. The CDN script-src is required because React +
// ReactDOM + Babel are loaded from unpkg; the eval below is required
// because the compiled JSX is executed via `(0, eval)`.
const REACT_PREVIEW_CSP = [
  "default-src 'none'",
    ...
    <script>
      (function(){
        ...
        try {
          ...
          // allow-same-origin, so the document is an opaque origin and cannot reach
          // parent storage. The CSP meta above adds a second layer: connect-src 'none'
          // blocks fetch / XHR / WebSocket / EventSource / Beacon, form-action 'none'
          // blocks form-based exfil, frame-src 'none' blocks clickjacking pivots.
          // 'unsafe-eval' is required for this line; 'unsafe-inline' is required for
          // this entire <script> block. See docs/prompt-engineering.md Layer 3.
          (0, eval)(compiled);
