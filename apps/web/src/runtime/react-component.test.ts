...
describe('buildReactComponentSrcdoc', () => {
  ...
  it('injects a Content-Security-Policy meta with the exfiltration-blocking directives', () => {
    const doc = buildReactComponentSrcdoc('export default () => <div />', { title: 't' });
    ...
    // The CSP must precede the React/Babel script tags so the directives
    // apply to those loads (not just to user code).
    const cspIndex = doc.indexOf('Content-Security-Policy');
    const reactScriptIndex = doc.indexOf('react.development.js');
    expect(cspIndex).toBeGreaterThan(-1);
    expect(reactScriptIndex).toBeGreaterThan(cspIndex);
  });
});
