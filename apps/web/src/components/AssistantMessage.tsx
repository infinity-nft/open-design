...
interface Props {
  ...
  // Fires when the user clicks "Auto-revise" on a lint findings card.
  // The text is the agent-ready feedback block from the daemon's
  // post-run linter (`agentMessage`); the parent forwards it to onSend
  // so the agent gets a structured system reminder describing what to
  // fix without the user having to type the diagnosis themselves.
  onAutoRevise?: (text: string) => void;
}
  ...
  return (
    <div className="msg assistant">
      ...
      <div className="assistant-flow">
        ...
        {blocks.map((b, i) => {
          if (b.kind === 'text')
            return (
              <ProseBlock
                key={i}
                text={stripStructuredBlocks(b.text)}
          ...
          if (b.kind === 'lint') {
            return (
              <LintFindingsCard
                key={i}
                payload={b.payload}
                canRevise={!streaming && !!isLast && !!onAutoRevise}
                onAutoRevise={onAutoRevise}
              />
            );
          }
          if (b.kind === 'critique') {
          ...
          if (b.kind === 'judge') {
            return <JudgeCard key={i} payload={b.payload} />;
          }
          return null;
        })}
        ...
        />
        {!streaming && !!isLast && message.runId ? (
          <FeedbackBar
            runId={message.runId}
            projectId={projectId}
            conversationId={null}
...
function dimLabel(short: 'P' | 'H' | 'D' | 'F' | 'I'): string {
...
function JudgeCard({
  ...
  payload: Extract<AgentEvent, { kind: 'judge' }>;
}) {
...
function CritiqueCard({
  ...
  payload: Extract<AgentEvent, { kind: 'critique' }>;
  canRevise: boolean;
  onAutoRevise?: ((text: string) => void) | undefined;
}) {
  const { verdict, reasoning, findings, totals, scores } = payload;
  const [expanded, setExpanded] = useState(verdict === 'revise');
  const tone = verdict === 'ship' ? 'p2' : totals.p0 > 0 ? 'p0' : 'p1';
  const summary = [
    totals.p0 > 0 ? `${totals.p0} P0` : null,
    totals.p1 > 0 ? `${totals.p1} P1` : null,
    totals.p2 > 0 ? `${totals.p2} P2` : null,
  ]
    .filter(Boolean)
    .join(' · ');
  ...
  const buildAgentMessage = (): string => {
    const lines = [
      '<artifact-critique>',
      `Self-critique verdict: ${verdict}.`,
      reasoning ? `Reasoning: ${reasoning}` : '',
      `${totals.p0} P0, ${totals.p1} P1, ${totals.p2} P2 finding(s).`,
    ];
    if (scores) {
      lines.push(
        `Dimension scores — Philosophy:${scores.philosophy} Hierarchy:${scores.hierarchy} Detail:${scores.detail} Functionality:${scores.functionality} Innovation:${scores.innovation}`,
      );
    }
    lines.push(
      'Re-emit a corrected `<artifact>`. Fix every P0 first, then P1. Do not rebuild the page wholesale — change only the affected sections.',
      '',
    );
    for (const f of findings) {
      lines.push(`**[${f.severity}] ${f.rule || 'unanchored'}** — ${f.evidence || '(no evidence cited)'}`);
      if (f.fix) lines.push(`  Fix: ${f.fix}`);
      lines.push('');
    }
    lines.push('</artifact-critique>');
    return lines.filter((l) => l !== undefined).join('\n');
  };

  const scoreColor = (n: number): string => {
    if (n >= 8) return 'critique-score--high';
    if (n >= 5) return 'critique-score--mid';
    return 'critique-score--low';
  };

  return (
    <div className={`lint-card lint-card--${tone}`} data-critique="true">
      <div className="lint-card-header">
        <button
          type="button"
          className="lint-card-toggle"
          onClick={() => setExpanded((x) => !x)}
          aria-expanded={expanded}
        >
          <span className={`lint-chip lint-chip--${tone}`}>
            {verdict === 'revise' ? 'Self-critique: revise' : 'Self-critique: ship'}
          </span>
          {scoreEntries ? (
            <span className="critique-scores">
              {scoreEntries.map(([dim, val]) => (
                <span key={dim} className={`critique-score ${scoreColor(val)}`} title={dimLabel(dim)}>
                  {dim}·{val}
                </span>
              ))}
            </span>
          ) : summary ? (
            <span className="lint-summary">{summary}</span>
          ) : null}
          <span className="lint-toggle-icon">{expanded ? '▾' : '▸'}</span>
        </button>
        {canRevise && verdict === 'revise' && findings.length > 0 && onAutoRevise ? (
          <button
            type="button"
            className="lint-revise-btn"
            onClick={() => onAutoRevise(buildAgentMessage())}
            title="Send the critique back to the agent so it can self-correct"
          >
            Auto-revise
          </button>
        ) : null}
      ...
      {expanded ? (
        <div className="lint-card-body">
          {reasoning ? <div className="lint-reasoning">{reasoning}</div> : null}
          {findings.length === 0 ? (
            <div className="lint-empty">No findings — the agent considers this artifact ready to ship.</div>
          ) : (
            <ul className="lint-finding-list">
              {findings.map((f, j) => (
                <li key={`${f.rule}-${j}`} className={`lint-finding lint-finding--${f.severity.toLowerCase()}`}>
                  <span className="lint-severity">{f.severity}</span>
                  <span className="lint-finding-message">
                    <strong>{f.rule || 'unanchored'}</strong>
                    {f.evidence ? ` — ${f.evidence}` : ''}
                  </span>
                  {f.fix ? <span className="lint-fix">{f.fix}</span> : null}
                </li>
              ))}
            </ul>
          )}
...
function LintFindingsCard({
  payload,
  canRevise,
  onAutoRevise,
}: {
  payload: Extract<AgentEvent, { kind: 'lint' }>;
  canRevise: boolean;
  onAutoRevise?: ((text: string) => void) | undefined;
}) {
  const [expanded, setExpanded] = useState(payload.hasP0);
  const { totals, artifacts, hasP0, hasP1, agentMessage } = payload;
  const summary = [
    totals.p0 > 0 ? `${totals.p0} P0` : null,
    totals.p1 > 0 ? `${totals.p1} P1` : null,
    totals.p2 > 0 ? `${totals.p2} P2` : null,
    ...
    .filter(Boolean)
    .join(' · ');
  const tone = hasP0 ? 'p0' : hasP1 ? 'p1' : 'p2';
  return (
    <div className={`lint-card lint-card--${tone}`}>
      <div className="lint-card-header">
        <button
          type="button"
          className="lint-card-toggle"
        ...
        >
          <span className={`lint-chip lint-chip--${tone}`}>
            {hasP0 ? 'AI-slop detected' : hasP1 ? 'Style hints' : 'Polish hints'}
          </span>
          <span className="lint-summary">{summary}</span>
          <span className="lint-toggle-icon">{expanded ? '▾' : '▸'}</span>
        </button>
        {canRevise && agentMessage.length > 0 && hasP0 && onAutoRevise ? (
          <button
            type="button"
            className="lint-revise-btn"
            onClick={() => onAutoRevise(agentMessage)}
            title="Send the findings back to the agent so it can self-correct"
          >
            Auto-revise
          </button>
        ) : null}
      ...
      {expanded ? (
        <div className="lint-card-body">
          {artifacts.map((a) => (
            <div key={a.relPath} className="lint-artifact">
              <div className="lint-artifact-path">{a.relPath}</div>
              <ul className="lint-finding-list">
                {a.findings.map((f, j) => (
                  <li key={`${f.id}-${j}`} className={`lint-finding lint-finding--${f.severity.toLowerCase()}`}>
                    <span className="lint-severity">{f.severity}</span>
                    <span className="lint-finding-message">{f.message}</span>
                    <span className="lint-fix">{f.fix}</span>
                  </li>
                ))}
              </ul>
            </div>
...
type Block =
  ...
  | { kind: 'lint'; payload: Extract<AgentEvent, { kind: 'lint' }> }
  | { kind: 'critique'; payload: Extract<AgentEvent, { kind: 'critique' }> }
  | { kind: 'judge'; payload: Extract<AgentEvent, { kind: 'judge' }> };
...
function buildBlocks(events: AgentEvent[]): Block[] {
  ...
  for (const ev of events) {
    ...
    if (ev.kind === 'lint') {
      out.push({ kind: 'lint', payload: ev });
      continue;
    }
    if (ev.kind === 'critique') {
      // Last critique wins — the agent can emit two (initial + revised
      // artifact) but the chat surface only needs to show the verdict
      // it landed on.
      const last = out[out.length - 1];
      if (last && last.kind === 'critique') {
        last.payload = ev;
      } else {
        out.push({ kind: 'critique', payload: ev });
      }
      continue;
    }
    if (ev.kind === 'judge') {
      const last = out[out.length - 1];
      if (last && last.kind === 'judge') last.payload = ev;
      else out.push({ kind: 'judge', payload: ev });
      continue;
    }
  }
  return out;
...
/**
 * Strip `<od-critique>` and `<od-brief>` blocks from prose so they do
 * not leak into the rendered text. They are surfaced via the typed
 * critique event and the brief-editor overlay respectively.
 */
function stripStructuredBlocks(text: string): string {
  return text
    .replace(/<od-critique\b[^>]*>[\s\S]*?<\/od-critique\s*>/gi, '')
    .replace(/<od-brief\b[^>]*>[\s\S]*?<\/od-brief\s*>/gi, '')
    .replace(/<od-judge\b[^>]*>[\s\S]*?<\/od-judge\s*>/gi, '')
    .replace(/\n{3,}/g, '\n\n');
}
