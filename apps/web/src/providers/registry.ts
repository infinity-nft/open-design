...
export interface DaemonReadiness {
  /** True when the process answers HTTP at all. */
  live: boolean;
  /** True when the daemon is *also* fully ready (db open, ≥1 agent). */
  ready: boolean;
  /** Per-check breakdown when /health/ready answers; null if unreachable. */
  checks: { db: boolean; agents: boolean; agentCount: number } | null;
  /** Structured reasons when ready is false. */
  reasons: string[];
}

/**
 * Probe both health endpoints. /health/live distinguishes "process up" from
 * "fully ready," so the chat surface can render different states:
 *   - live=false               → "Daemon disconnected" (red)
 *   - live=true, ready=false   → "Starting up" (amber, with reasons)
 *   - live=true, ready=true    → no badge
 *
 * Both fetches share a 1.5s timeout so a stuck daemon does not freeze
 * the badge poller.
 */
export async function daemonReadiness(): Promise<DaemonReadiness> {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 1500);
  try {
    const [liveResp, readyResp] = await Promise.allSettled([
      fetch('/health/live', { signal: ctrl.signal }),
      fetch('/health/ready', { signal: ctrl.signal }),
    ]);
    const live =
      liveResp.status === 'fulfilled' && liveResp.value.ok;
    if (readyResp.status !== 'fulfilled') {
      return { live, ready: false, checks: null, reasons: live ? [] : ['daemon unreachable'] };
    }
    const body = await readyResp.value.json().catch(() => null);
    if (!body || typeof body !== 'object') {
      return { live, ready: false, checks: null, reasons: ['health probe returned non-JSON'] };
    }
    const ready = Boolean((body as { ready?: unknown }).ready);
    const checks = (body as { checks?: unknown }).checks as DaemonReadiness['checks'] | undefined;
    const reasons = Array.isArray((body as { reasons?: unknown }).reasons)
      ? ((body as { reasons?: unknown[] }).reasons!.filter((r) => typeof r === 'string') as string[])
      : [];
    return { live, ready, checks: checks ?? null, reasons };
  } catch {
    return { live: false, ready: false, checks: null, reasons: ['daemon unreachable'] };
  } finally {
    clearTimeout(timer);
  }
}
